...
Contains the static HTML renderer for built books. The renderer is split by responsibility:

- `renderer.mbt`
  render entrypoint, filesystem writes, asset copying, and renderer tests
- `renderer_shell.mbt`
  page shell, navigation, 404 page, page links, and path helpers
- `renderer_markdown_blocks.mbt`
...
- `renderer_assets_refs.mbt`
  markdown asset discovery, raw HTML attribute rewriting, link references, and footnotes

### `cmd/main/`

Contains CLI dispatch. `main.mbt` stays small and routes subcommands to focused handlers:
...
- `extensions_install.mbt`
  extension file-copy and seed-tree installation helpers
- `extensions_config.mbt`
  manifests, providers, identity, routines, memory, and runtime config templates
- `extensions_profiles.mbt`
...
- `ingest_semantics.mbt`
  entity, concept, claim, relationship, and ontology extraction helpers for ingest
- `ingest_source_utils.mbt`
  source classification, source-signal detection, path normalization, slug, and link utilities
- `query.mbt`
...
- `ui_state.mbt`
  generated-site state aggregation and projection entrypoints
- `ui_state_content.mbt`
  research/marketing content extraction and evidence parsing for generated projections
- `ui_state_projection_html.mbt`
  generated research, marketing, journal, course, and skill HTML/CSS/JS renderers
- `skill_hub_assets.mbt`
  Skill Hub HTML/CSS/JS asset templates separated from server/state logic
- `skills.mbt`
  public skill inventory/install/compare/sync/scaffold operations
- `skills_types.mbt`
  skill manager data contracts
- `skills_inventory.mbt`
  skill discovery, metadata parsing, relationships, and diagnostics
- `skills_compare.mbt`
  selector, baseline resolution, and diff summaries
- `skills_files.mbt`
  path, copy/sync, slug, and scaffold helpers
- `workspace.mbt`
