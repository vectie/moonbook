...
- exposing a native CLI flow for `init`, `build`, `load`, `test`, and `clean`
...
- end-to-end `init`, `build`, `load`, `test`, `clean`, and `version` commands
...
Moonbook currently behaves like a partial mdBook replacement for straightforward books. It does not yet provide full upstream behavior for themes, search, preprocessors/plugins, `serve/watch`, print output, or the complete markdown/rendering edge-case surface of Rust mdBook.
