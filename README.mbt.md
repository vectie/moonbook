...
- 🎓 a repository-to-coursebook skill and standalone site template for features,
  architecture, data flow, operation, debugging, complete English/简体中文
  lessons, real product screenshots, verified deployment recipes, and a bounded
  documentation pet
...
- `2026-08-12`: rebuilt `wiki-course` as a newcomer-first repository-to-coursebook workflow with seven standard learning journeys, complete bilingual projections, real-state screenshots, exercised copy-paste recipes, an opt-in Technical notes view, a responsive standalone site template, freshness checks, and an optional read-only MoonClaw documentation pet
...
- 🎓 seeded `skills/wiki-course/SKILL.md` plus a reusable standalone coursebook
  site for turning a software repository into newcomer-first feature,
  architecture, data-flow, usage, operations, and debugging lessons while
  keeping source provenance and readiness judgment in explicit Technical notes
