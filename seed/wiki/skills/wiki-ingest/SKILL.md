...
MoonBook should express research ingest as a four-phase semantic graph:
...
`marketing_project` is intentionally separate from research ingest and from
`knowledge_revise`. Schedule it only for an explicit marketing or website-
projection goal after the research envelope has passed review.
...
It is generated by the separate, optional `marketing_project` task after the
evidence matrix, synthesis brief, and deep report exist and have been reviewed.
It should name:
...
10. return the raw artifact envelope so MoonBook can materialize durable wiki pages

When an explicit marketing or website-projection goal exists, schedule the
separate `marketing_project` task after review and write `marketing-brief.md`
through the wiki-marketing skill. Do not add it to the research phase chain.
